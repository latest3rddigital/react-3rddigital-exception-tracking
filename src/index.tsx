...
type InternalConfig = Required<
  Pick<
    ...
    | "enabled"
    | "reactTrackingEnabled"
    | "capacitorTrackingEnabled"
...
const isTrackingEnabledForSource = (source: ExceptionSource) => {
...
const safePluginCall = async <T,>(getter: () => Promise<T>) => {
  try {
    return await getter();
  } catch {
    return undefined;
...
const installGlobalHandlers = () => {
  ...
  const rejectionHandler = (event: PromiseRejectionEvent) => {
    if (
      !currentConfig?.captureUnhandledRejections ||
